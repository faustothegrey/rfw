...
class IptablesTest(TestCase):
    ...
    def test_find(self):
        ...
        r5 = Rule(chain='PREROUTING', num='11', pkts='0', bytes='0', target='DNAT', prot='tcp', opt='--', inp='*', out='*', source='0.0.0.0/0', destination='7.7.7.6', extra='')
        ...
        self.assertEqual( inst1.find({'chain': ['PREROUTING'], 'target': ['DNAT']}), [r5])
        self.assertEqual( inst1.find({'chain': ['OUTPUT', 'INPUT'], 'target': ['ACCEPT']}), [r2])
        self.assertEqual( inst1.find({'chain': ['OUTPUT', 'INPUT', 'PREROUTING'], 'target': ['ACCEPT', 'DROP', 'DNAT']}), rules)
